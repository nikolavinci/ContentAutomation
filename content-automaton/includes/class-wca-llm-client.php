...
class NCA_LLM_Client {
    ...
    private function call_openai( $prompt ) {
        ...
        $content = isset( $body["choices"][0]["message"]["content"] ) ? $body["choices"][0]["message"]["content"] : false;
        $tokens = isset( $body["usage"]["total_tokens"] ) ? $body["usage"]["total_tokens"] : 0;
        
        if ($content) return array("content" => $content, "tokens" => $tokens, "cost" => ($tokens / 1000) * 0.015);
        return false;
    }
    ...
    private function call_gemini( $prompt ) {
        ...
        $body = json_decode( wp_remote_retrieve_body( $response ), true );
        $content = isset( $body["candidates"][0]["content"]["parts"][0]["text"] ) ? $body["candidates"][0]["content"]["parts"][0]["text"] : false;
        
        // Gemini API token counting is separate, rough estimate:
        $tokens = str_word_count($content) * 1.3;
        
        if ($content) return array("content" => $content, "tokens" => $tokens, "cost" => ($tokens / 1000) * 0.007);
        return false;
    }
}
